...
const SHIMMER = {
  backgroundImage: `linear-gradient(90deg, ${withAlpha(BRAND_SAND, 0.55)} 40%, ${withAlpha(BRAND_WHITE, 0.98)} 50%, ${withAlpha(BRAND_SAND, 0.55)} 60%)`,
  backgroundSize: "250% 100%",
} as const;

// the placeholder grid is sand, deepening toward the sand hairline, with a
// whisper of ink in the darkest cells so it never reads as flat paper
const TONE_LIGHT = hexToRgb255(BRAND_SAND);
const TONE_DARK = hexToRgb255(BRAND_SAND_STROKE);
const TONE_INK = hexToRgb255(BRAND_INK);
const INK_BLEED = 0.06;
...
// always light: the portal has no dark theme
function toneOf(tone: number, clock: number): Rgb255 {
  const t = clamp01(tone * 0.9 + Math.sin(clock * 1.5 + tone * 6.28) * 0.05);
  const ink = INK_BLEED * t;
  return [
    mix(mix(TONE_LIGHT[0], TONE_DARK[0], t), TONE_INK[0], ink),
    mix(mix(TONE_LIGHT[1], TONE_DARK[1], t), TONE_INK[1], ink),
    mix(mix(TONE_LIGHT[2], TONE_DARK[2], t), TONE_INK[2], ink),
  ];
}
...
function drawScene(s: Scene) {
  ...
  const shade = (channel: number, target: number) =>
    Math.round(mix(channel, target, tint));
  const base = toneOf(root.tone, s.clock);

  // gutters recess into this instead of cutting through to the surface behind
  ctx.fillStyle = `rgb(${Math.round(shade(base[0], root.r) * 0.92)},${Math.round(
    shade(base[1], root.g) * 0.92,
  )},${Math.round(shade(base[2], root.b) * 0.92)})`;
  ...
  const paint = (p: Patch) => {
    ...
    const tone = toneOf(p.tone, s.clock);
    ctx.fillStyle = `rgb(${shade(tone[0], p.r)},${shade(tone[1], p.g)},${shade(tone[2], p.b)})`;
...
export function GridReveal({
  ...
  return (
    <div
      ...
      className={cn(
        "relative w-full overflow-hidden rounded-16 bg-sand-base [corner-shape:squircle]",
        className,
      )}
    ...
    >
      ...
      {caption ? (
        <motion.div
          layout
          className="pointer-events-none absolute bottom-3 left-3 flex h-6 items-center overflow-hidden bg-strong-950/45 px-2.5 backdrop-blur-md"

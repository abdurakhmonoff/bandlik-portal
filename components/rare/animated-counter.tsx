...
function useWheel(
  ...
  return useTransform(
    pos,
    (p: number) => `${(-mod(p, 10) * 100) / WHEEL.length}%`,
  );
}
...
> & {
  ...
  suffix?: ReactNode;
  /**
   * Renders 0 until this first turns true, then counts up to `value` once.
   * Drive it from an IntersectionObserver. Scrolling back out does not reset
   * it — after the first reveal the counter tracks `value` as usual.
   */
  inView?: boolean;
};
...
export function AnimatedCounter({
  ...
  // once revealed it stays revealed, so leaving the viewport does not rewind.
  // adjusted in render rather than an effect: an effect here would paint 0 first
  const [revealed, setRevealed] = useState(inView);
  if (inView && !revealed) setRevealed(true);

  const target = revealed ? value : 0;
  const shape = measure(target, decimals, padStart, duration);
  ...
  return (
    <span
      ...
      className={cn(
        "inline-flex items-center font-display tabular text-strong-950",
        className,
      )}
      {...props}
    >

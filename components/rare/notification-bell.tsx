...
const SURFACE = "bg-white-0 border border-soft-200 hover:bg-weak-50";
const GLYPH = "text-strong-950";
...
export type NotificationBellColor = keyof typeof COLORS;
...
function DigitColumn({ value, reduced }: { value: number; reduced: boolean }) {
  ...
  const y = useTransform(position, (p: number) => `${-p * 100}%`);
  const velocity = useVelocity(position);
  const mask = useTransform(velocity, (v: number) => {
    const fade = Math.min(ROLL_FADE, (Math.abs(v) / ROLL_VELOCITY) * ROLL_FADE);
    return `linear-gradient(to bottom, transparent 0%, rgba(0,0,0,1) ${fade}%, rgba(0,0,0,1) ${100 - fade}%, transparent 100%)`;
  });
    ...
    <AnimatePresence initial={false}>
      {total > 0 && (
        ...
        >
          {!dot && (
            <span
              className="flex font-semibold leading-none tracking-tight text-static-white"
...
> & {
  ...
  color?: NotificationBellColor;
  asChild?: boolean;
  /** Replaces the default bell glyph. It still swings when the count rises. */
  icon?: ReactNode;
  children?: ReactNode;
};

// what cloneElement needs to know about whatever `asChild` was handed
type SlottableProps = {
  className?: string;
  style?: CSSProperties;
  children?: ReactNode;
...
export function NotificationBell({
  ...
  // a local slot rather than @radix-ui/react-slot: one cloneElement is all
  // asChild needs here, and it keeps the component dependency-free
  if (asChild) {
    if (!isValidElement<SlottableProps>(children)) return null;
    const child = children as ReactElement<SlottableProps>;

    return cloneElement(child, {
      ...props,
      "data-slot": "notification-bell",
      className: cn("relative", child.props.className, className),
  ...
  return (
    <button
      ...
      data-slot="notification-bell"
      className={cn(
        "focus-ring relative grid place-items-center rounded-full outline-none transition-transform active:scale-90 motion-reduce:active:scale-100",
        SURFACE,
        GLYPH,
        className,
      )}
    ...
    >
      ...
      {icon ? (
        <motion.span
          aria-hidden
          className="grid place-items-center [&_svg]:block"
          style={{
            width: side,
            height: side,
            rotate: swing,
        ...
        <BellIcon side={side} swing={swing} clapper={clapper} />
      )}
      {badge}
    </button>
  );

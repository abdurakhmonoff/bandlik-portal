...
import { BRAND_INK, BRAND_WHITE } from "./brand";
...
// the seam svg paints with currentColor, so the text colour has to be the bar colour
const BAR = "bg-sand-base";
const BAR_TEXT = "text-sand-base";
...
function Segment({
  ...
  const d = useTransform(marginLeft, (g: number) => neckPath(g, span));
...
function NavLabel({
  ...
  const props = {
    ...
    className: cn(
      // min-h-11 keeps the 44px tap target whatever the size prop trims off
      "flex min-h-11 cursor-pointer items-center whitespace-nowrap font-medium [&_svg]:shrink-0",
      isActive ? FADE_IN : FADE_OUT,
      SIZES[size].label,
      !isActive && "text-sub-600 hover:text-strong-950",
    ),
...
export function GooeyNav({
  ...
  onChange,
  size = "md",
  activeColor = BRAND_INK,
  activeLabelColor = BRAND_WHITE,
  separation,
  radius,
  className,
  ...props
  ...
  return (
    <nav
      data-slot="gooey-nav"
      className={cn("max-w-full overflow-x-auto no-scrollbar", className)}
      {...props}
    >

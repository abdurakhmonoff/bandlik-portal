...
> & {
    ...
    seekable?: boolean
    /** aria-labels for the transport button. Defaults are Uzbek. */
...
const TransportIcon = ({ state, size, reduced }: { state: IconState; size: number; reduced: boolean }) => {
    ...
    const shapeScale = useTransform(shapeOpacity, (o: number) => ENTER_SCALE + (1 - ENTER_SCALE) * o)
    const leavingScale = useTransform(leavingOpacity, (o: number) => ENTER_SCALE + (1 - ENTER_SCALE) * o)
    ...
    useEffect(() => {
        ...
        if (canMorph(from, state)) {
            ...
            const controls = animate(0, 1, {
                ...ICON_SPRING,
                onUpdate: (t: number) => shape.set(morph(Math.min(1, Math.max(0, t)))),
...
function StepPlayer({
    ...
    const text = useMemo<StepPlayerLabels>(() => ({ ...DEFAULT_LABELS, ...labels }), [labels])
    ...
    const fillWidth = useTransform(progress, (p: number) => `${p * 100}%`)
    ...
    return (
        <div
            ...
            className={cn(
                'inline-flex items-center text-sub-600',
        ...
        >
            {showControl && (
                <motion.button
                    ...
                    aria-label={finished ? text.replay : isPlaying ? text.pause : text.play}
                    ...
                    className="flex shrink-0 cursor-pointer touch-manipulation select-none items-center justify-center rounded-full bg-strong-950 text-static-white outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-base"
                >
            ...
            <div
                ...
                className="flex flex-1 items-center justify-center rounded-full bg-sand-base"
            >
                {items.map((step, i) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className={cn(
                                ...
                                state === 'past' ? 'bg-primary-base' : 'bg-sand-stroke',
                                seekable &&
                                    'has-focus-visible:outline-2 has-focus-visible:outline-offset-4 has-focus-visible:outline-primary-base',
                            )}
                        >
                            {state === 'active' && (
                                <motion.span
                                    ...
                                    className="absolute inset-y-0 left-0 bg-primary-base"
                                />
                            )}
                            {seekable && (
                                <button

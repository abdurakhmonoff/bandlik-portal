...
import { BRAND_GOLD, BRAND_PRIMARY, hexToRgb01 } from "./brand";

export type FluidOrbProps = React.ComponentProps<"div"> & {
  size?: number;
  /** Base churn colour. */
  color?: string;
  /** Second churn colour; the fbm value blends `color` into this one. */
  color2?: string;
  /** Stops the render loop — pass true while the orb is offscreen. */
  paused?: boolean;
};
...
void main() {
  ...
  // the churn itself decides the hue, so both colours travel through the orb
  float blend = smoothstep(0.25, 0.75, f);
  vec3 accent = mix(u_color, u_color2, blend);
  ...
  vec3 light = mix(white, accent, 0.5);
  vec3 dark = accent;
...
// local rather than motion's hook: the orb is otherwise dependency-free
...
const FluidOrb = ({
  ...
  // set by the GL effect, read by the loop effect below it
  ...
  useEffect(() => {
    ...
    gl.uniform3f(gl.getUniformLocation(program, "u_color"), ...hexToRgb01(color));
    gl.uniform3f(
    ...
    drawRef.current = (seconds: number) => {
      gl.uniform1f(uTime, seconds);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
    };
    // paint once here so a paused or reduced-motion orb is never a blank canvas
    // after a resize or a colour change; the loop below overwrites it if running
    drawRef.current(0);
  ...
  useEffect(() => {
    // reduced motion and paused both settle on the same static frame
    if (reduced || paused) {
      drawRef.current?.(0);
      return;
    }
